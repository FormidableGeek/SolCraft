...
export function MyBalanceCard({ className }: MyBalanceCardProps) {
        ...
        try {
          ...
          if (userDocSnap.exists()) {
            ...
            setDisplayBalance({
              ...
              walletAddress: profileData.walletAddress ?? 'Loading...'
            });
          } else {
            toast({ title: "Profile not found", description: "Could not load balance details.", variant: "destructive" });
          ...
          setDisplayBalance({ amount: 0, currency: 'USD', walletAddress: 'Error' });
        }
      } else {
        setAuthUser(null);
  ...
  const handleCopyAddress = () => {
    if (displayBalance.walletAddress && displayBalance.walletAddress !== 'N/A' && displayBalance.walletAddress !== 'Loading...' && displayBalance.walletAddress !== 'Error') {
  ...
  if (isLoading) {
    return (
      <Card className={cn("flex flex-col items-center justify-center", className)} style={{minHeight: '150px'}}>
    ...
    <Card className={cn(className)}>
      ...
      <CardContent className="text-center">
        ...
        <div className="flex items-center justify-center text-xs text-muted-foreground">
          ...
          <Button variant="ghost" size="icon" className="ml-1 h-6 w-6" onClick={handleCopyAddress} disabled={displayBalance.walletAddress === 'N/A' || displayBalance.walletAddress === 'Loading...'}>
